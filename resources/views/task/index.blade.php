...
<section class="w-full">
    {{-- Browse Tasks --}}
    <section class="w-full h-fit px-4 sm:px-5">
        <div class="mb-3 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 class="font-bold text-2xl text-gray-800">{{ __('Browse Tasks') }}</h2>
        ...
        <div class="w-full mt-8 mb-5">
            @forelse($tasks_in_progress as $key => $project)
                ...
                @if($projectModel)
                    <div class="mb-6">
                        <h3 class="font-bold text-xl text-gray-700 my-6 mx-2 border-l-4 border-violet-500 pl-4">
                            {{ $projectModel->name }}
                        </h3>
            ...
            @empty
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                    ...
                    <p class="text-gray-500 text-lg">{{ __("No tasks in progress yet.") }}</p>
                </div>
            @endforelse
        </div>
    </section>
